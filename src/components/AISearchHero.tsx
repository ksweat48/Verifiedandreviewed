      ...
      {!showResults && (
        <div className="flex flex-col justify-center items-center flex-grow z-10 px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col items-center text-center max-w-4xl mx-auto">
            ...
            {/* Initial Hero Content - Show when not searching */}
            {!isSearching && (
              <>
                {/* Title and Subtitle */}
                <div className="mb-8">
              ...
              {results.map((business, index) => (
                business.isPlatformBusiness ? (
                  <PlatformBusinessCard
                    ...
                    matchPercentage={getMatchPercentage(business.similarity)}
                    index={index}
                  />
                ) : (
                  <AIBusinessCard
      ...
      {showResults && results.length === 0 && !isSearching && (
        <div className="flex-1 flex items-center justify-center bg-neutral-50 px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-md">
            <Icons.Search className="h-12 w-12 text-neutral-400 mx-auto mb-4" />
            <h3 className="font-cinzel text-xl font-semibold text-neutral-900 mb-2">
              No Results Found
            </h3>
            <p className="font-lora text-neutral-600 mb-4">
              We couldn't find any businesses matching your search. Try a different query or check your location settings.
            </p>
            <button
              onClick={() => {
                setSearchQuery('');
              ...
              className="bg-primary-500 text-white px-4 py-2 rounded-lg font-poppins font-semibold hover:bg-primary-600 transition-colors duration-200"
            >
              Try Another Search
            </button>
          </div>
