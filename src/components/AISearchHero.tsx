...
// Distance options for the slider (removed 15 miles)
  ...
  const handleSearch = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (finalBusinesses.length === 0) {
        setError(`No businesses found within ${maxRadius} miles. Try expanding your search radius or using different keywords.`);
      }
                ...
                {error ? (
                  ...
                  <div className="h-full flex items-center justify-center p-4">
                    <div className="bg-white/95 backdrop-blur-sm rounded-2xl p-6 text-center max-w-md">
                      ...
                      <p className="font-lora text-neutral-600 mb-4">
                        No businesses found within {maxRadius} miles. Try expanding your search radius or using different keywords.
                      </p>
                      <button
                        onClick={handleExitAppMode}
                        className="font-poppins bg-primary-500 text-white px-4 py-2 rounded-lg font-semibold hover:bg-primary-600 transition-colors duration-200"
                    ...
                    <div className="md:hidden h-full flex items-center justify-center px-4">
                      <div className="w-full max-w-sm">
                        {businesses[currentBusinessIndex] && (
                          <div>
                            {businesses[currentBusinessIndex].isPlatformBusiness ? (
                              <PlatformBusinessCard
                                business={businesses[currentBusinessIndex]}
                                onRecommend={handleRecommendBusiness}
                                onTakeMeThere={handleTakeMeThere}
                              />
                            ) : (
                        ...
                        <div className="text-center mt-4">
                          <p className="font-lora text-xs text-white/70">
                            Swipe left/right to browse • {currentBusinessIndex + 1} of {businesses.length}
                          </p>
                        </div>
                      </div>
                    </div>
